...
@app.cell
def _(Any, NDArray, Path, cv, dataclass, defaultdict, np):
    ...
    class ViewEdge:
        i: int
        j: int
        inliers_ij: int  # num inlier matches i -> j
        inliers_ji: int  # num inlier matches j -> i
        matches_ij: list[cv.DMatch] | None = None
        matches_ji: list[cv.DMatch] | None = None
    ...
    class ViewGraph:
        ...
        def add_edge(self, i, j, inliers_ij, inliers_ji, matches_ij=None, matches_ji=None):
            """
    ...
    def construct_view_graph(kp: list, des: list, K):
        ...
        # matches_ij, matches_ji = [], []
        for i in range(N):
            for j in range(i + 1, N):
                ok_ij, inliers_ij, match_ij = has_overlap(kp[i], des[i], kp[j], des[j], K)
                ok_ji, inliers_ji, match_ji = has_overlap(kp[j], des[j], kp[i], des[i], K)
                # ASK: why the matches should not be preserved ???
                if ok_ij and ok_ji:
                    # matches_ij.append(match_ij)
                    # matches_ji.append(match_ji)
                    view_graph.add_edge(i, j, inliers_ij, inliers_ji, matches_ij=match_ij, matches_ji=match_ji)
...
def _(
    ...
    def add_view(img_new: ImageData, img_ref: ImageData, K, dist, track_manager: TrackManager, point_cloud: PointCloud):
        ...
        pnp_ok, rvec, tvec, inliers = cv.solvePnPRansac(object_points, img_new_pts_tracked, K, dist)
    ...
    return PointCloud, TrackManager, compute_baseline_estimate
...
def _(
    ...
    #         print(f"{R = }\n{U = }\n")
    #         R.remove(best_edge.i if best_edge.i in R else best_edge.j)
    #         U.remove(best_edge.i if best_edge.i in U else best_edge.j)
    #         print(f"{R = }\n{U = }\n")
    #         print(f"Failed to add view: {img_new.path.name} with ref: {img_ref.path.name} due to {e}")
    #         continue
    ...
    # point_cloud.save_ply(filename=Path(f"{img_dir.name}.ply"))
    ...
    return best_edge, img_0, img_1


@app.cell
def _(best_edge, cv, img_0, img_1):
    import matplotlib.pyplot as plt

    # print(len(view_graph.edges) == len(matches_ij))
    # print(len(view_graph.edges) == len(matches_ji))
    print(best_edge.i == img_0.idx)
    print(best_edge.j == img_1.idx)

    img_matches = cv.drawMatches(
        img_0.img, img_0.kp,
        img_1.img, img_1.kp,
        best_edge.matches_ij, None,
    ...
    ax0.axis("off")
    ax0.set_title(f"{img_0.idx=} {img_1.idx=}")
