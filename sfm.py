...
class TrackManager:
    ...
    def update_tracks(self, img_idx_new: int, img_idx_ref: int, ref2new_matches):
        ...
        return kp_idx_new_tracked, matches_untracked


def compute_baseline_estimate(img_0: ImageData, img_1: ImageData, K, track_manager):
    ...
    # TODO: img_0.set_pose(np.eye(3), np.zeros(3))
...
def add_view(
    img_new: ImageData, img_ref: ImageData, K, dist, track_manager, point_cloud
):
    ...
    track_ids, kp_idx_new_tracked, matches_untracked = track_manager.update_tracks(
        img_new, img_ref, matches
    )
    # Only use tracked KPs for reconstruction; tracked == "have 3D point"
    # TODO: PointCloud should be managed separately, TM should index into it;
    # PointCloud.export(), plot_colored(), plot_depth()?
    object_points = point_cloud.get_object_points(track_ids)

    # Estimate pose of new image camera from tracked KPs and their 3D pts
    # PnP needs tracked KPs from new image (2D) and matching 3D pts
    img_new_pts = np.float32([img_new.kp[i].pt for i in kp_idx_new_tracked])
    pnp_ok, R, t, inliers = cv.solvePnPRansac(object_points, img_new_pts, K, dist)
    if not pnp_ok:
...
def main():
    ...
    _, _, points_3d = compute_baseline_estimate(img_0, img_1, K, track_manager)
    ...
    while True:
        ...
        _, _, points_3d = add_view(img_new, img_ref, K, dist, track_manager)
